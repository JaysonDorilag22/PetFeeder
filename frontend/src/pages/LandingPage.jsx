...
function LandingPage() {
  ...
  return (
    ...
    <div className='pt-5 '>
    <h2 className="text-3xl font-extrabold text-yellow-500">GROUP 2 MEMBERS</h2>
    </div>
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 justify-center pt-5">
  {images.map((image, index) => (
    <div key={index} className="text-center">
      <img
        className="w-full h-auto max-w-xs max-h-64 object-cover rounded-lg"
        src={image.src}
        alt={`Gallery image ${index + 1}`}
      />
      <p className='pt-2 font-extrabold'>{image.name}</p>
      <p className='pt-2 font-light'>{image.roles}</p>
    </div>
  ))}
</div>
